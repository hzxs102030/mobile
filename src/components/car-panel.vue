<template>
	<li class="nav-cart" @mouseenter="showcarButton" @mouseleave="hidecarButton" style="position: fixed;right: -5px;" @click="carshow=!carshow">
		...
		<div class="nav-cart-wrapper" v-if="carshow" style="width: 300px;">
			<div class="nav-cart-list">
				
				<div class="empty" v-if="count==0">
					<h3>收藏夹为空</h3>
					<p>您还没有选购任何商品，现在前往商城选购吧!</p>
					<el-button-group style="position: absolute;width: 300px;left: -3px;bottom: 0px;">
					  <router-link class="nav-cart-btn" :to="{name:'People',query:{show1:true}}" style="font-size: 16px;">
					  	<el-button plain size="small" style="width: 98px; border-radius: 0px;color:green"><i class="el-icon-s-custom"></i>个人中心</el-button>
					  </router-link>
					  <router-link class="nav-cart-btn" :to="{name:'People',query:{show5:true}}" style="font-size: 16px;">
					  	<el-button plain size="small" style="width: 98px; border-radius: 0px;color:cadetblue;"><i class="el-icon-wind-power"></i>联系地址</el-button>
					  </router-link>
					  <router-link class="nav-cart-btn" to="/car" style="font-size: 16px;">
						  <el-button plain size="small" style="width: 99px; border-radius: 0px;color: plum;"><i class="el-icon-shopping-cart-2"></i>收藏夹</el-button>
					  </router-link>
        ...
        overflow-y: scroll;height:255px;">
						<ul>
							<li class="clear" v-for="item,index in carPanelData">
								<div class="cart-item js-cart-item cart-item-sell" style="width: 270px;">
									<div class="cart-item-inner">
										<div class="item-thumb">
											<img :src="item.ali_image+'?x-oss-process=image/resize,w_80/quality,Q_100/format,webp'">
										</div>
										<div class="item-desc" style="position: absolute;top:0px;left: 25px;">
											<div class="cart-cell" >
												<h4 style="position: absolute;top:20px;left:-10px"> 
												<router-link :to="{name:'Item',query:{itemId:item.sku_id}}" style="color:coral;">{{item.title}}</router-link>
													
												</h4>
												<p class="attrs">
													<span>{{item.spec_json.show_name}}</span>
												</p>
												<h6 style="position: absolute;top:45px;left: 80px;">
													<span class="price-icon">单价：¥</span><span class="price-num">{{item.price}}/晚</span><span class="item-num">{{item.count}}</span>
												</h6>
											</div>
										</div>
										<div class="el-icon-circle-close" @click="delPanelHandle(item.sku_id)" style="font-size: 20px;
										color: red;position: absolute;top:20px;left:250px;"></div>
									</div>
								</div>
							</li>
						</ul>
		...
		>
			<div class="addcart-mask" v-show="ball.show">
				<img class="mask-item" style="z-index: 999;"/>
...
carshow:false,
		...
		methods:{
			...
			beforeEnter(el){
				...
				let y=rect.top+rect.height/2-rectCar.top
...
<style>
	.ball-enter-active{
		transition: 1s cubic-bezier(1.000, 0.060, 0.790, 0.875);
	}
